...
import { FaQuestionCircle, FaCheck, FaClock, FaGraduationCap } from 'react-icons/fa';

const FAQ = () => {
  const faqs = [
    {
      question: "Jak můžu zrušit členství?",
      answer: "Členství můžeš zrušit kdykoliv na pár kliků. GrowMat Academy je na platformě Flixy a tam je zrušení členství jednoduché. V případě jakýchkoliv problémů mi napiš a se zrušením Ti obratem pomůžu.",
      icon: <FaCheck className="text-green-400 text-xl" />
    },
    {
      question: "Jak rychle uvidím výsledky?",
      answer: "To je čistě na Tobě. Členové akademie mají první virální přípěvek do 14 dnů a s ním i prodeje. GrowMat Academy Ti dá systém a podporu, ať se můžeš k úspěšným členům připojit i ty :).",
      icon: <FaClock className="text-green-400 text-xl" />
    },
    {
      question: "Jsem úplný začátečník, zvládnu to?",
      answer: "Tak právě pro Tebe je GrowMat Academy jako dělaná. Provedu Tě od založení profilu po první prodeje na Instagramu krok za krokem a předám Ti veškeré potřebné know-how a nástroje. Pokud budeš mít dotaz - v sekci otázky osobně odpovídám a podporuji členy komunity k úspěchu.",
    ...
    <section className="relative bg-gradient-to-br from-gray-900 via-black to-gray-900 text-white py-24 px-4 overflow-hidden">
      ...
      <div className="relative max-w-5xl mx-auto z-10">
        ...
        <div className="space-y-8">
          {faqs.map((faq, index) => (
            <div key={index} className="group bg-white/5 backdrop-blur-md border border-gray-700/50 rounded-2xl p-8 hover:border-green-400/50 transition-all duration-300 hover:-translate-y-1">
